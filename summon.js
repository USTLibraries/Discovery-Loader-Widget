/*
	Discovery Loader Widget (Summon)
...
(function () {
	...
	console.log("DISCOVERY: Loading Discovery search box (Summon: ver "+ver+")...");
	...
	if (  typeof($) !== "undefined" ) {
		$(document).ready( function(){
				...
				$.fn.discoverySearchWidget = function() {
					...
					return this.each( function() {
						...
						$(searchForm).attr({
							...
							name: 'summonSearch',
							target: '_blank',
							action: discoveryCustom.url + '/search',
						...
						*/
						...
						if( isTrue(temp) ) {
							...
							if ( type === "BOOK" || type === "BOOKS" ) {
								for (var i = 0, len = discoveryCustom.facet_books.length; i < len; i++) {
									hiddenFields.push( createHiddenField("s.fvf[]","ContentType,"+discoveryCustom.facet_books[i]) );
								}								
							} else if ( type === "MUSIC" ) {
								for (var i = 0, len = discoveryCustom.facet_audio.length; i < len; i++) {
									hiddenFields.push( createHiddenField("s.fvf[]","ContentType,"+discoveryCustom.facet_audio[i]) );
								}	
							} else if ( type === "FILM" || type === "FILMS" ) {
								for (var i = 0, len = discoveryCustom.facet_video.length; i < len; i++) {
									hiddenFields.push( createHiddenField("s.fvf[]","ContentType,"+discoveryCustom.facet_video[i]) );
								}	
							} else {
						...
						if( isTrue(temp) ){
							...
							$(a).attr("href",discoveryCustom.url + "/#!/advanced");
							$(a).html(s);
						...
						if ( typeof($.fn.autocomplete) != "undefined" ) {
							...
							$( searchField ).autocomplete({
								source: function( request, response ) {
									$.ajax({
										url: discoveryCustom.url + "/metadata/suggest/suggest",
										dataType: "jsonp",
										data: {
											type_strict: 'should',
											all_types: 'true',
			...
			cssCheck(); // check to see if css is loaded
